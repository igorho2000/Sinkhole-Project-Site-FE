export const ch = {
	...
	"time-2": {
		...
		text3: "透過2019~2022年的資料，統計每月的地震數量，並結合其他變數，建立機器學習模型。結果顯示，地震數量是模型中第四重要的變數(共19個變數)。雖然人為因素如地基開挖，也有可能造成土壤鬆動，但多數情況是因擋土壁施作不良而導致掏空塌陷(詳後續空間分析)，不納入地震成因討論。",
	...
	"time-3": {
		...
		text2: "根據模型結果，月雨量在特徵重要性排名第二。然而進一步觀察雨量與掏空案件的關係，發現相關性僅低度正相關(0 - 0.3)。可能是單一成因不足以解釋掏空的發生，或者並非單純的雨量越多掏空就會越多，也許會有一個雨量達到多大才會增加掏空的複雜關係。目前工務局新工處針對雨量，也設下雨量達 77mm時進行道路巡檢的預防措施，預防掏空災害發生。",
